...
const sendClientReady = (isReconnect) => {
  ...
  // PURE USERNAME-BASED: Extract userName and roomName from URL parameters (NO TOKEN/COOKIE)
  ...
  if (!userNameFromUrl && encryptedUserName) {
    try {
      ...
      console.log(`[NO-TOKEN] Decrypted userName from share link: ${userNameFromUrl.substring(0, 3)}...`);
    } catch (e) {
      console.error('[NO-TOKEN] Failed to decrypt userName:', e);
  ...
  if (!roomNameFromUrl && encryptedRoomName) {
    try {
      ...
      console.log(`[NO-TOKEN] Decrypted roomName from share link: ${roomNameFromUrl.substring(0, 3)}...`);
    } catch (e) {
      console.error('[NO-TOKEN] Failed to decrypt roomName:', e);
